...
describes what users get. Pick a format with `-o`, or let refactoringguru choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
refactoringguru <command> -o table   # aligned columns for reading
refactoringguru <command> -o jsonl   # one JSON object per line, for piping
refactoringguru <command> -o json    # a single JSON array
refactoringguru <command> -o csv     # spreadsheet friendly
refactoringguru <command> -o tsv     # tab-separated
refactoringguru <command> -o url     # just the URL column
refactoringguru <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
refactoringguru <command> --fields id,title,url
```
...
```bash
refactoringguru <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
refactoringguru <command>            # a table, because this is a terminal
refactoringguru <command> | wc -l    # JSONL, because this is a pipe
```
