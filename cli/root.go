// Package cli builds the refactoringguru command tree.
package cli
...
import (
	...
	"github.com/tamnd/refactoringguru-cli/refactoringguru"
)
...
type App struct {
	client   *refactoringguru.Client
	cfg      refactoringguru.Config
	output   string
	fields   []string
	noHeader bool
	template string
...
func Root() *cobra.Command {
	app := &App{cfg: refactoringguru.DefaultConfig()}

	root := &cobra.Command{
		Use:           "refactoringguru",
		Short:         "Browse the Refactoring.Guru design-patterns catalog from the command line",
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = refactoringguru.NewClient(a.cfg)
	return nil
}
